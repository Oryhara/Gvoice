...
namespace Google.Voice
{
    public class Result
    {
        ...
        public bool isResponseValid(string testSMS, string smsResponse) {
            return false;

        }

        public string Content { get; private set; }
        public HtmlNodeCollection GetMessagesResponsesFromXml() {
            string responseXml = XML().SelectSingleNode("/response/html").InnerText;
            var doc = new HtmlDocument();
            doc.LoadHtml(responseXml);
            var messageNodes = doc.DocumentNode.SelectNodes("//div[@class=\"gc-message-sms-row\"]");
            return messageNodes;
        }
