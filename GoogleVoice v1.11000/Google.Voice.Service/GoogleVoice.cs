...
namespace Google.Voice
{
    public class GoogleVoice
    {
        ...
        /// </summary>
        public string gxf { get; protected set; }
        ...
        protected static string GenerateUrl(string method = null)
        {
            ...
            switch (method.ToLower())
            {
                case "login":
                    ...
                    return "https://accounts.google.com/ServiceLogin?service=grandcentral&passive=1209600&continue=https://www.google.com/voice&followup=https://www.google.com/voice&ltmpl=open";

                case "executelogin":
                    return "https://accounts.google.com/signin/challenge/sl/password?service=grandcentral&continue=https://www.google.com/voice/redirection/voice&followup=https://www.google.com/voice&ltmpl=open";
        ...
        /// <param name="pageNumber">Page number to retrieve</param>
        /// <param name="urlOverride">URL override, if retrieving from a different location</param>
        public HtmlNodeCollection GetLatestMessage(string rnr_se, uint? pageNumber = null, string urlOverride = null) {
            string base_url = (urlOverride ?? GenerateUrl());
            string get_url = base_url + "recent/";
            ...
            try {
                ...
                if (messageJson.totalSize == 0) {//no messages
                    ...
                    var messageId = doc.DocumentNode.SelectSingleNode("//div").Attributes["id"].Value;
                    delete["messages"].Value = messageId;
                    delete["trash"].Value = 1;
                    delete["_rnr_se"].Value = rnr_se;
                    string archive_url = base_url + "deleteMessages/";
                    var response = WebClient.Post(archive_url, delete);
                    return messages;
                }
            } catch (Exception ex) {
        ...
        public static implicit operator string(GoogleVoice response) {
